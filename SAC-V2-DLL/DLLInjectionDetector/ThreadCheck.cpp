...
void LdrInitializeThunk_t(PCONTEXT NormalContext, PVOID SystemArgument1, PVOID SystemArgument2)
{
	...
	{
		ErrorHandler::ErrorMessage("0x2342 ( Please Restart )", 5);
	}
	...
	if (GetModuleInformation(NtCurrentProcess, LoadLibraryA("user32"), &user32ModInfo, sizeof(user32ModInfo)))
	{
		...
		{
			ErrorHandler::ErrorMessage("0x4001 ( Please Restart )", 5);
	...
	{
		ErrorHandler::ErrorMessage("0x4006 ( Please Restart )", 5);
	...
	{
		ErrorHandler::ErrorMessage("0x4007 ( Please Restart )", 5);
	...
	{
		ErrorHandler::ErrorMessage("0x4008 ( Please Restart )", 5);
	...
	{
		ErrorHandler::ErrorMessage("0x4009 ( Please Restart )", 5);
		//ErrorHandler::ErrorMessage("10", 5);
	
		//printf("# WARNING # dwStartAddress == RtlCreateUserThread\n");
	}
	else if (dwStartAddress == (DWORD)GetProcAddress(LoadLibraryA("ntdll"), "CreateRemoteThread"))
	{
		ErrorHandler::ErrorMessage("0x7432 ( Please Restart )", 5);
		//ErrorHandler::ErrorMessage("10", 5);
		//printf("# WARNING # dwStartAddress == RtlCreateUserThread\n");
	}
	else if (dwStartAddress == (DWORD)GetProcAddress(LoadLibraryA("ntdll"), "CreateRemoteThreadEx"))
	{
		ErrorHandler::ErrorMessage("0x8932 ( Please Restart )", 5);
		//ErrorHandler::ErrorMessage("10", 5);
		//printf("# WARNING # dwStartAddress == RtlCreateUserThread\n");
	}
	...
	if (VirtualQuery((LPCVOID)dwStartAddress, &mbi, sizeof(mbi)))
	{
		...
		{
			ErrorHandler::ErrorMessage("0x4010 ( Please Restart )", 5);
			//ErrorHandler::ErrorMessage("13", 5);
		}

		if (dwStartAddress == (DWORD)mbi.AllocationBase)
		{
			ErrorHandler::ErrorMessage("0x4011 ( Please Restart )", 5);
	...
	{
		ErrorHandler::ErrorMessage("0x4012 ( Please Restart )", 5);
		//ErrorHandler::ErrorMessage("15", 5);
	}

	if (CUtils::GetThreadOwnerProcessId(dwThreadId) != GetCurrentProcessId())
	{
		ErrorHandler::ErrorMessage("0x4013 ( Please Restart )", 5);
	...
	if (pCurrentSecHdr)
	{
		...
		if (IsMonitored)
		{
			//ErrorHandler::ErrorMessage("0x25321 ( Please Restart )", 5);
		}
	}
...
void InitializeThreadCheck()
{
	...
	{
		ErrorHandler::ErrorMessage("0x4014 ( Please Restart )", 5);
